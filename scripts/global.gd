...
var player_team: int = 0  # Add this - defaults to first team (red)

signal battle_mode_toggled(enabled: bool)
signal health_bars_toggled(enabled: bool)
signal admin_mode_toggled(enabled: bool)
...
func toggle_admin_mode() -> void:
	admin_mode = !admin_mode
	admin_mode_toggled.emit(admin_mode)
